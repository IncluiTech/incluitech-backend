...
public class SolucionadorController {
  ...
  @PostMapping
  public ResponseEntity<SolucionadorResponse> salvar(
      @RequestBody SolucionadorRequest solucionadorRequest) {
    log.info("Salvando solucionador: {}", solucionadorRequest);
    Solucionador solucionadorSalvo =
        service.salvar(SolucionadorMapper.mapToModel(solucionadorRequest));
