...
public class BotEngineTest {
  ...
  public void setup() {
    this.botEngine =
        new BotEngine(
            Arrays.asList(new TestClienteMockedConjutoRegras(), new TestDefaultConjuntoRegras()));
  }
  ...
  public void shouldSelectClienteConjutoRegrasWhenUserTypeIsCliente() {
    ...
    Optional<BotMessage> message =
        conjuntoRegra.flatMap(regra -> regra.processaRegras(mensagemInterna));
    // then
    ...
    Assert.assertEquals(
        ComponentBotMessageType.BOTAO, message.get().getMessages().get(0).getType());
  }
  ...
  public void shouldAlterContext() {
    ...
    Assert.assertEquals(1, message.get().getMessages().size());
    Assert.assertEquals(
        ComponentBotMessageType.TEXTO, message.get().getMessages().get(0).getType());
