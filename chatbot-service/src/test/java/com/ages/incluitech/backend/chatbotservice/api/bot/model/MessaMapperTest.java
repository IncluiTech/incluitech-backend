...
public class MessaMapperTest {
  ...
  public void shouldTranslateUserTextMessageToTextInternalMessage() throws IOException {
    ...
    // when:
    List<MensagemInterna> mensagemInterna =
        MessageMapper.mensagemDoUsuarioParaMensagemInterna(userMessage);
    // then:
  ...
  public void shouldTranslateBotTextMessageToFacebookSenderMessage() {
    ...
    List<FacebookMessage> messages =
        MessageMapper.botMessageParaFacebookMessage(
            Collections.singletonList(botMessage), mensagemInterna);
    // then:
  ...
  public void shouldTranslateBotButtonMessageToFacebookSenderMessage() {
    ...
    MensagemInterna mensagemInterna = buildInternalMessage();
    List<Button> buttons =
        Arrays.asList(new PayloadButton("titulo1", "CONTEUDO_1"), new LinkButton("titulo2", "url"));
    ...
    Assert.assertEquals(
        "button", messages.get(0).getMessage().getAttachment().getPayload().getTemplateType());
    ButtonsAttachment buttonsAttachment =
        convertAttachemntPayloadTo(ButtonsAttachment.class, messages);
    ...
    Assert.assertEquals(
        "CONTEUDO_1", ((PayloadButton) buttonsAttachment.getButtons().get(0)).getPayload());
  ...
  public void shouldTranslateBotQuickReplyMessageToFacebookSenderMessage() {
    // given:
    MensagemInterna mensagemInterna = buildInternalMessage();
    ComponentBotMessage botMessage =
    ...
    Assert.assertEquals(
        "Titulo", messages.get(0).getMessage().getQuickRepliesButtons().get(0).getTitle());
    Assert.assertEquals(
        "conteudo", messages.get(0).getMessage().getQuickRepliesButtons().get(0).getPayload());
    Assert.assertEquals(
        "Titulo1", messages.get(0).getMessage().getQuickRepliesButtons().get(1).getTitle());
    Assert.assertEquals(
        "conteudo1", messages.get(0).getMessage().getQuickRepliesButtons().get(1).getPayload());
  }
  ...
  public void shouldTranslateBotCarouselMessageToFacebookSenderMessage() {
    // given:
    MensagemInterna mensagemInterna = buildInternalMessage();
    List<Button> buttonsFirstCard =
        Collections.singletonList(new PayloadButton("titulo", "CONTEUDO_1"));
    List<Button> buttonsSecondCard =
        Collections.singletonList(new PayloadButton("titulo", "CONTEUDO_1"));
    ComponentBotMessage botMessage =
        new CarrouselComponentBotMessage(
            Arrays.asList(
                new Card()
                    .withTitle("FirstCard")
                    .withSubtitle("FirstCardSubTitle")
                    .withButtons(buttonsFirstCard),
                new Card()
                    .withTitle("SecondCard")
                    .withSubtitle("SecondCardSubTitle")
                    .withButtons(buttonsSecondCard)));
    // when:
    List<FacebookMessage> messages =
        MessageMapper.botMessageParaFacebookMessage(
            Collections.singletonList(botMessage), mensagemInterna);
    ...
    Assert.assertNotNull(messages.get(0).getMessage().getAttachment());
    Assert.assertEquals(
        "generic", messages.get(0).getMessage().getAttachment().getPayload().getTemplateType());
    CarouselAttachmentPayload carouselComponentBotMessage =
        convertAttachemntPayloadTo(CarouselAttachmentPayload.class, messages);
    ...
    Assert.assertEquals(
        "FirstCardSubTitle", carouselComponentBotMessage.getElements().get(0).getSubtitle());
    Assert.assertEquals(
        buttonsFirstCard, carouselComponentBotMessage.getElements().get(0).getButtons());
    Assert.assertEquals("SecondCard", carouselComponentBotMessage.getElements().get(1).getTitle());
    Assert.assertEquals(
        "SecondCardSubTitle", carouselComponentBotMessage.getElements().get(1).getSubtitle());
    Assert.assertEquals(
        buttonsSecondCard, carouselComponentBotMessage.getElements().get(1).getButtons());
  }

  private MensagemInterna buildInternalMessage() {
    return new MensagemInterna(new UsuarioDaMensagem("2242"), TipoMensagem.TEXTO, "conteudoTest");
  }
  ...
  private UserMessage getUserMessage(String fileName) throws IOException {
    ...
    File file =
        new File(
            Objects.requireNonNull(loader.getResource("usermessages/" + fileName + ".json"))
                .getFile());
