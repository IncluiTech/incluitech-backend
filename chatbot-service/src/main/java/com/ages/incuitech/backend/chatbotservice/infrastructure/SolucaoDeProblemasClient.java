...
public class SolucaoDeProblemasClient {
  ...
  public void saveSolucionador(SolucionadorRequest solucionadorRequest) {
    ...
    log.info(
        String.format(
            "Iniciando chamada REST para solucao-de-problemas-service para salvar solucionador:  %s",
            solucionadorRequest));

    try {
      restTemplate.postForEntity(
          properties.getUrl() + properties.getUri(), request, SolucionadorRequest.class);
  ...
  public void updateSolucionador(SolucionadorRequest solucionadorRequest) {
    ...
    log.info(
        String.format(
            "Iniciando chamada REST para solucao-de-problemas-service para atualizar solucionador:  %s",
            solucionadorRequest));
  ...
  public SolucionadorRequest getByFacebookId(String facebookId) {
    ResponseEntity<SolucionadorRequest> response =
        restTemplate.getForEntity(
            properties.getUrl() + properties.getUri() + "/" + facebookId,
            SolucionadorRequest.class);
