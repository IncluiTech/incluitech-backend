...
public class EspecificacoesRegra implements RegraDoBot {
  ...
  public BotMessage processa(MensagemInterna message) {
    if (this.isPrimeiraInteracao(message.getConteudo())) {
      ...
      return confirmarTags
          ? this.confirmarTags(message)
  ...
  private List<String> getTagsFromContexto(MensagemInterna message) {
    return (List<String>)
        message.getContexto().getOrDefault("areasAtuacao", new ArrayList<String>());
  }
  ...
  private BotMessage perguntarSobreAreaDeAtuacao(MensagemInterna message) {
    ...
    List<QuickReplyButton> buttons =
        defaults.stream().map(tag -> new QuickReplyButton(tag, tag)).collect(toList());
    return new BotMessage(message.getContexto())
        .withMessages(
            new QuickReplyComponentBotMessage(
                "Por favor, selecione uma especificação abaixo", buttons));
  }
