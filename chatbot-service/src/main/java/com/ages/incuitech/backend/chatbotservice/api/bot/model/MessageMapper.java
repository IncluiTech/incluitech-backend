...
public class MessageMapper {
  ...
  public static List<FacebookMessage> botMessageParaFacebookMessage(
      List<ComponentBotMessage> componentBotMessages, MensagemInterna mensagemInterna) {
    return componentBotMessages
        .stream()
        .map(
            componentBotMessage -> {
              FacebookMessage facebookMessage =
                  constroiFacebookMessage(componentBotMessage, mensagemInterna.getUsuario());
              if (TipoMensagem.EVENTO.equals(mensagemInterna.getTipo())) {
                return facebookMessage
                    .withMessageType(MessageType.MESSAGE_TAG.name())
  ...
  public static List<MensagemInterna> mensagemDoUsuarioParaMensagemInterna(UserMessage message) {
    return message
        ...
        .flatMap(
            entry ->
                entry.getMessaging().stream().map(MessageMapper::converteMensagemUsarioParaInterna))
        .collect(Collectors.toList());
  }

  private static FacebookMessage constroiFacebookMessage(
      ComponentBotMessage componentBotMessage, UsuarioDaMensagem usuarioDaMensagem) {
    if (componentBotMessage instanceof ButtonComponentBotMessage) {
      return constroiMenssagemDeBotoes(
          (ButtonComponentBotMessage) componentBotMessage, usuarioDaMensagem.getId());
    } else if (componentBotMessage instanceof CarrouselComponentBotMessage) {
      return constroiMenssagemDeCarrossel(
          (CarrouselComponentBotMessage) componentBotMessage, usuarioDaMensagem.getId());
    } else if (componentBotMessage instanceof QuickReplyComponentBotMessage) {
      return constroiMenssagemQuickReplyParaFacebookMenssage(
          (QuickReplyComponentBotMessage) componentBotMessage, usuarioDaMensagem.getId());
    } else {
      return constroiTextBotMessageParaFacebookMessage(
          (TextComponentBotMessage) componentBotMessage, usuarioDaMensagem.getId());
    }
  }

  private static FacebookMessage constroiTextBotMessageParaFacebookMessage(
      TextComponentBotMessage textMessage, String userId) {
    TemplateMessage template = TemplateMessage.builder().text(textMessage.getText()).build();
    return new FacebookMessage().withRecipient(new UserRecipient(userId)).withMessage(template);
  }

  private static FacebookMessage constroiMenssagemQuickReplyParaFacebookMenssage(
      QuickReplyComponentBotMessage quickReply, String userId) {
  ...
  private static FacebookMessage constroiMenssagemDeCarrossel(
      CarrouselComponentBotMessage carrousel, String userId) {
    TemplateMessage template =
        TemplateMessage.builder()
            .attachment(
                new AttachmentBotMessage(new CarouselAttachmentPayload(carrousel.getElementos())))
            .build();
    return new FacebookMessage().withRecipient(new UserRecipient(userId)).withMessage(template);
  }

  private static FacebookMessage constroiMenssagemDeBotoes(
      ButtonComponentBotMessage buttonMessage, String userId) {
    TemplateMessage template =
        TemplateMessage.builder()
            .attachment(
                new AttachmentBotMessage(
                    new ButtonsAttachment(buttonMessage.getTexto(), buttonMessage.getBotoes())))
            .build();
    return new FacebookMessage().withMessage(template).withRecipient(new UserRecipient(userId));
  }
  ...
  public static MensagemInterna converteApiMessageParaInterna(ApiMessage messaging) {
    ...
    return new MensagemInterna(
        usuarioDaMensagem, TipoMensagem.EVENTO, messaging.getConteudo(), new HashMap<>());
  }

  private static boolean hasAttachment(Messaging messaging) {
    return messaging.getMessage().getAttachments() != null
        && !messaging.getMessage().getAttachments().isEmpty();
  }
}
