...
public class BotService {
  ...
  public void manipulaEvento(MensagemInterna mensagemInterna) {
    ...
    facebookSendService.sendMessage(
        MessageMapper.botMessageParaFacebookMessage(botMessage.getMessages(), mensagemInterna));
  }

  @Async
  public void manipulaMensagem(List<MensagemInterna> mensagensInternas) {
    mensagensInternas.forEach(
        mensagem -> {
          UsuarioDaMensagem usuarioDaMensagem =
              userService.getUsuario(mensagem.getUsuario().getId());
          ...
          facebookSendService.sendMessage(
              MessageMapper.botMessageParaFacebookMessage(botMessage.getMessages(), mensagem));
        });
  }
  ...
  private BotMessage constroiMensagemDefault() {
    List<ComponentBotMessage> componentBotMessages =
    ...
    List<ComponentBotMessage> componentBotMessages =
        Collections.singletonList(
            new TextComponentBotMessage(
                "Opa, algo de errado aconteceu, por favor, tente mais tarde"));
    return new BotMessage(new HashMap<>(), componentBotMessages);
  }
}
