...
public class ContatoInformadoRegra implements RegraDoBot {
  ...
  private BotMessage handleEmailETelefone(
      MensagemInterna message, Map<String, Object> contexto, String contato) {
    boolean isEmailDefined =
        Objects.nonNull(message.getContexto().get(TipoContato.EMAIL.getPropriedade()));
    boolean isTelefoneDefined =
        Objects.nonNull(message.getContexto().get(TipoContato.TELEFONE.getPropriedade()));
  ...
  private BotMessage pedirTelefoneParaUsuario(Map<String, Object> contexto) {
    ...
    return new BotMessage(contexto)
        .withMessages(new TextComponentBotMessage("Agora, insira seu telefone:"));
  }
    ...
    return new BotMessage(contexto)
        .withMessages(
            new QuickReplyComponentBotMessage(
                "Antes de seguirmos para o seu perfil, me diga, o que você procura?",
                new QuickReplyButton("Busco soluções", TipoUsuario.CLIENTE.getTipo()),
                new QuickReplyButton(
                    "Busco resolver problemas", TipoUsuario.SOLUCIONADOR.getTipo())));
  }
}
